...
| 1-14 | 能否表示为标量势的梯度/矢量势的旋度 | 若 $\mathbf F=\nabla u$ ，则必须有 $\nabla\times\mathbf F=0$ ；若 $\mathbf F=\nabla\times\mathbf A$ ，则必须有 $\nabla\cdot\mathbf F=0$ |
...
### 6. 为什么是“写成梯度/旋度”，不是“写成散度”

1-14 判断的是一个**矢量场**能否由势函数生成：

$$
\text{标量势 }u\xrightarrow{\nabla}\nabla u
\quad\text{（结果是矢量）},
$$

$$
\text{矢量势 }\mathbf A\xrightarrow{\nabla\times}
\nabla\times\mathbf A
\quad\text{（结果也是矢量）}.
$$

散度 $\nabla\cdot\mathbf F$ 的结果是标量，只表示源分布，不能在本题中拿来表示原来的矢量场。判断时之所以使用另一个算子，是因为
...
所以想写成梯度，要先检查原场的旋度是否为零；想写成旋度，要先检查原场的散度是否为零。反向结论还需要适当的区域和光滑性条件。

教材印刷页 20 的式（1.6.12）在内层 $\nabla$ 与 $\varphi$ 之间多印了一个点。若按 $\nabla\cdot\varphi$ 理解，就变成“微分算子与标量做散度”，运算类型不成立。教材印刷页 14 的式（1.4.9）已经定义 $\operatorname{grad}u=\nabla u$ ，而印刷页 20 紧随公式的文字也写的是“标量场梯度的旋度”，因此式（1.6.12）的正确形式应为
